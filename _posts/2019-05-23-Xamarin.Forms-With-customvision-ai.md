...
subtitle:  微软Custom Vision集成到Xamarin.Forms APP #副标题
date:       2019-05-23              # 时间
author:     BY 碧雪晴天                     # 作者
...
我们唯一要做的事情，就是上传图片并使用正确的标签对图片进行标记，可以在短时间内，

对上千张图片进行标记。一旦模型训练完成，我们就可以将训练完成的模型根据我们的自定义需求，集成到我们的程序中。目前Custom Vision支持如下图离线模型导出，并支持在线API访问。
